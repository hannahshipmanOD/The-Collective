...
**Pre-closing navigation:** Ops Hub Tasks (`/tasks`) > filter Type to the transaction task types, Due = Overdue
...
**Navigation:** Ops Hub Tasks (`/tasks`) or Dashboard > set the Assignee filter to the HSA name > search through and claim tasks (open a task > claim it).

**Viewing Claimed Tasks:** In Ops Hub Tasks/Dashboard, set Assignee to "Me" (or another individual's name for coverage). Complete a task from its detail page > "Mark complete."
...
**Action:**
1. Contact the customer and reschedule the IWT through Ops Hub.
2. Navigate to the deal page Assessments section (`#assessments`) > click "Schedule Assessment" > select "In-person walkthrough" > pick an available time slot > click "Schedule."
...
3. Upon receiving documentation: File a "Septic Inspection Review" Zendesk ticket from the deal page (Ops Hub Tasks section `#todos` > Add task > Add ticket). Attach the seller's documentation.
...
**Script:**
> Hi [CX Name], I'm reaching out on behalf of our title team. They let us know the land survey provided is missing [fill in from task details]. In order for them to move forward with closing, you will need to provide a new survey with that information included. Please feel free to reach out to them directly at [insert email address from Ops Hub] if you have any additional questions. Thank you!
...
Ops Hub deal page > right-rail `⋯` overflow menu > "Customer Dashboard" (opens/copies the seller's dashboard link).
...
3. If no LCO previously set up: leave a note in Ops Hub indicating customer disclosed tenant-occupied home, close task.

**How to Cancel LCO:**
Ops Hub deal page > Checkout section (`#checkout`) > scroll to the "Late Checkout" subsection > cancel the Late Checkout > note LCO cancelled due to tenant > Slack title to inform them.
...
4. Ping the escrow officer noted in the Participants panel in Ops Hub.
5. Include property address. Ask if clear to close/on track for COE date.
6. Leave a note in Ops Hub: "Reached out to title to confirm closing on track [INSERT SLACK LINK TO MESSAGE]"
...
**DCO Approval Steps in Ops Hub:**
1. Click the task (opens the deal page, Checkout section).
...
6. Leave a note in Ops Hub: "Approved DCO."
7. Auto-email goes to title company approving DCO and they release funds.

**Scoping Review:**
- If large items appear in DCO photos, check Ops Hub > Assessments section (`#assessments`) to see if items were scoped during assessment.
...
2. Ops Hub deal page > "Contract Change" > "Request Approval - Other" > submit for HSA Manager approval.
...
**Approving LCO in Ops Hub:**
1. Click the task (opens the Checkout section).
...
6. Click "Approve."
7. Leave a note in Ops Hub: "Approved LCO."
...
**Overdue LCO Script:**
> Hi [CX Name], I'm following up on your move out photos, they were due at 5pm yesterday upon moving out. We will need them ASAP this morning to ensure there are no delays in your closing. Please let me know if you need any help, here's the link to your dashboard to submit them. [PASTE DASHBOARD LINK FROM OPS HUB]
...
3. If no: note in Ops Hub and close task manually.
...
- First check the Ops Hub property Overview page (not the Acquisition deal page) for existing tickets.
- If related ticket exists: leave internal note with additional info.
- If no related ticket: Ops Hub deal page > Tasks section (`#todos`) > Add task > Add ticket > HOA Violations > Describe the violation > Click "Create."
...
1. In the Ops Hub Checkout section (`#checkout`): update the move-out date.
...
5. Note in Ops Hub how many extra days used and how much withheld from deposit.
...
- Ops Hub deal page > "Request approval - other" under Contract Change. Include context.
- Use sparingly.
...
1. Ops Hub deal page > Tasks section (`#todos`) > Add task > Add ticket.
2. Select "Solar Escalation Acquisition" from the form dropdown.
...
1. Ops Hub Participants panel > the customer's `⋯` menu > "Email" to open the email in Zendesk.
...
1. When you receive the Solar ticket prompting kickoff, open the Ops Hub deal page and send the kickoff email.
...
4. Key: Internal notes on customer message page auto-post to the Ops Hub property/deal page. Notes on the Solar team ticket alone do NOT.
5. Mark solved and refresh the Ops Hub deal page to confirm note is showing on the property.
...
| Title channels (market-specific) | Confirm closing, title issues. Ping EO from the Ops Hub Participants panel. |
...
| Topic | Also See |
|-------|---------|
| Ops Hub navigation, task types, deal pages | `opshub-nav/SKILL.md`, `opshub-nav/UI_GUIDE.md` |
| City entity schemas (data lineage — still valid) | `opendoor-data/domains/city.md` |
