...
5. **Seller signs** → diligence period begins (Addendum §4: 5 days to schedule, visit within 10 days)
...
### When a seller asks “why is the service fee X%?” (verified 2026-08-03)

Do **not** assume they saw a % on the prelim dashboard. Run this gate first:

1. **Stage:** `offer.state` + `finalOfferViewedAt` + OpsHub flags (Estimated/SENT vs FO/ACCEPTED).
2. **Prelim / estimate hub** (`AssessmentStep` / `HoldingOfferStep` / `UnderwritingStep` → Cake `TreatmentOffer`): shows **Estimated Home Value / Up to $…** only — **no** “Opendoor service fee” line, **no** fee %.
3. **Final Offer** (`ContractSigningStep` / `RepricingStep`): line item **“Opendoor service fee” as $**; tooltip says fee “is a percentage of your offer price” **without printing the number**.
4. **SoT for the $ / fraction:** opshub GraphQL `serviceChargeCents` / `serviceChargePercentage` (live) or `ACQ_L2.PRELIM_*` / `FINALIZED_*` fee columns (laggy). Sellers often derive “15–16%” by dividing fee $ by home-value / listing estimate / headline.
5. **Surface checklist before claiming source:** SMS thread → Cake stage UI (above) → OpsHub Pricing (mirrors FO $; may be empty at Estimated) → Iterable only if a **per-user** key exists (campaign catalog alone ≠ proof this seller got the email) → Help Center (points to Offer Details $).
6. **Confidence tags:** data-validated vs UI-validated vs inferred — never ship “seller sees X” on data fields alone.
...
| **Inspection period** | Same Addendum §4 as Cash: 5 days to schedule, visit within 10 days |
| **Cancel policy** | Cancel anytime before CP1 closes at no cost |
| **List price cap (CP2)** | Seller's chosen resale list price is capped at **120% of Opendoor's estimated price** — reduced from 150% on 2026-06-25 (OFFER-2158). A seller wanting to list higher than 120% of the estimate can't; this bounds the CP2 upside they can chase. |
...
### How to Identify Cash Plus in Ops Hub
...
- **FO held for HSA:** Shows as `final_offer_unreleased` in Ops Hub
...
Source: T&O Sales Dashboard blessed `cep_metrics.sql` (updated 2026-02-26) — full detail in `opendoor-data/domains/hsa-performance.md` §6. Supersedes the task-based Feb 4, 2026 Sales Conversion Metric Definition doc.

```
FO Conversion = Contracts Signed / FO Properties (in period)
- Denominator: Unique flip_tokens with UW_COMPLETED_AT per month from ACQ_L2_FLIP_DETAILS, walks excluded (IS_WALKED = FALSE)
- Numerator: Contracts signed (PURCHASE_AGREEMENT_COMPLETED_AT IS NOT NULL) from that FO cohort
- Cohorting: One flip_token per month — first UW completion date wins (per Mike Billett 2026-02-06: "HSA starts working on a final offer once UW is Complete")
- HSA attribution: AX_FLIP_PARTICIPANTS.acq_ae_id
- Settlement lag: 5 days after period end
- Filters: Channel, True Seller, Serious Seller
```

⚠️ The older task-based version (Contracts Signed / FO Tasks Created, cohorted on task creation date) is valid for task-level analysis only — it is NOT the blessed dashboard metric used for HSA performance.
...
- **Diligence period is not product-split.** Opendoor Addendum §4 (V21, on Cash and CNML): Seller has **5 days** from the Addendum date to schedule the diligence visit, and may pick any available day within a **10-day window**. One reschedule inside that window with 1 day's notice. If the visit does not occur within 10 days (unless Buyer and Seller explicitly agree otherwise), the Contract is deemed canceled by Buyer and earnest money goes to Buyer. **Do not say CNML is 15 days** — that was old internal language; the CNML-specific addendum is fees (program fee + initial cash adjustment), not a longer diligence clock.
...
- **HSA can schedule the DV through Ops Hub** (deal page → Assessments section `#assessments` → Schedule Assessment) if seller needs help
...
| Day 10 | Contract window ends (Addendum §4). Warning SMS. If visit has not occurred and parties have not agreed otherwise → deemed canceled; EMD to Buyer. |
| Day 13–15 | **Stale ops sequence** in older playbooks (urgent call / auto-cancel Day 15). That is **not** current Addendum language. Confirm live Iterable/HSA tasks before citing. |
...
3. Leave a note in Ops Hub with the ZD ticket number
...
**How to identify:** Check for the **"New Final Offer Prez"** tag on the transaction in Ops Hub.
...
Standard DV credit addendum process still applies — use Contract Change → Create Generic Addendum from the Ops Hub deal page.
...
**Creating the Addendum in Ops Hub:**

1. Pull up the Acquisition Transaction (deal) page
...
- **Notice of Cancellation** auto-generates in Ops Hub when contract is withdrawn
...
2. Include: Ops Hub link, flip token, headline price, amount requested, context
3. **David Villasenor** reviews and responds
4. Approval thresholds: < $3K manager discretion, > $5K requires conversation
5. HSA applies in Ops Hub (CR state) or via Zendesk ticket (pre-contract)
...
Step 2B: PROCESS / TIMELINE
  ├── FO delay                  → Check Ops Hub state, route to UW if stuck
...
- **Ops Hub tags:** "Instant Offer" and/or "Instant Final Offer"
...
| Ops Hub tags | "Instant Offer" / "Instant Final Offer" | "Lennar Trade In lead", "Lennar Trade In" |
